...
# Lane Configuration
bsu_road_distance = 10
PB_road_distance = 24
Bulaklakan_road_distance = 20
Hdel_road_distance = 20
...
# Red and blue lines for Bulaklakan
Bulaklakan_red_line_y, Bulaklakan_blue_line_y = 300, 215
Bulaklakan_line_start_1, Bulaklakan_line_end_1 = (0, PB_red_line_y), (1080, PB_red_line_y)
Bulaklakan_line_start_2, Bulaklakan_line_end_2 = (0, PB_blue_line_y), (1080, PB_blue_line_y)

# Red and blue lines for Hdel
Hdel_red_line_x, Hdel_blue_line_x = 650, 250
Hdel_line_start_1, Hdel_line_end_1 = (Hdel_red_line_x, 0), (Hdel_red_line_x, 1080)
Hdel_line_start_2, Hdel_line_end_2 = (Hdel_blue_line_x, 0), (Hdel_blue_line_x, 1080)
...
bsu_roi = np.array([
    [425, 180],     # Bottom left
    [580, 180],    # Bottom right
    [500, 100],    # Top right
    [430, 100]     # Top left
])

Bulaklakan_roi = np.array([
    [250, 500],  # Bottom left 
...
vehicle_classifications_Bulaklakan = {class_id: 0 for class_id in allowed_classes}
vehicle_classifications_Hdel = {class_id: 0 for class_id in allowed_classes}
...
unique_vehicle_count_Bulaklakan = [0]
unique_vehicle_count_Hdel = [0]

async def process_video(video_path, line_start_1, line_end_1, line_start_2, line_end_2, speed_calculator, latest_speed_store, unique_vehicle_counter, roi):
    ...
    try:
        while cap.isOpened():
            ...
            # cv2.polylines(frame, [bsu_roi], isClosed=True, color=(0, 255, 0), thickness=2)
            # cv2.polylines(frame, [Bulaklakan_roi], isClosed=True, color=(0, 255, 0), thickness=2)
            # cv2.polylines(frame, [Hdel_roi], isClosed=True, color=(0, 255, 0), thickness=2)
            ...
            rects = []
            class_id = -1 
            class_name = "No Vehicle Detections"
            display_speed = 0.0
            ...
            if len(results) > 0 and len(results[0].boxes) > 0:
                ...
                # Assign detected class
                class_id = valid_detections[0][2]
                ...
                for obj in objects_bbs_ids:
                    ...
                    if class_id in allowed_classes:
                        ...
                        elif r'video_samples\2.mp4' in video_path:
                            if class_id not in crossed_vehicles:
                                vehicle_classifications_Bulaklakan[class_id] += 1
                        elif r'video_samples\4.mp4' in video_path:
                            if class_id not in crossed_vehicles:
                                vehicle_classifications_Hdel[class_id] += 1
...
def PB_video_feed():
    ...
    return StreamingResponse(process_video(
        ...
        PB_speed_calculator, latest_speed_PB, unique_vehicle_count_PB, pb_roi
...
def bsu_video_feed():
    ...
    return StreamingResponse(process_video(
        ...
        bsu_speed_calculator, latest_speed_bsu, unique_vehicle_count_BSU, bsu_roi
    ), media_type="multipart/x-mixed-replace; boundary=frame")
    
    
@app.get("/Bulaklakan_video_feed")
def Bulaklakan_video_feed():
    global unique_vehicle_count_PB
    return StreamingResponse(process_video(
        r'video_samples\2.mp4',
        Bulaklakan_line_start_1, Bulaklakan_line_end_1, Bulaklakan_line_start_2, Bulaklakan_line_end_2,
        Bulaklakan_speed_calculator, latest_speed_Bulaklakan, unique_vehicle_count_Bulaklakan, Bulaklakan_roi
    ), media_type="multipart/x-mixed-replace; boundary=frame")


@app.get("/Hdel_video_feed")
def Hdel_video_feed():
    global unique_vehicle_count_BSU
    return StreamingResponse(process_video(
        r'video_samples\4.mp4',
        Hdel_line_start_1, Hdel_line_end_1, Hdel_line_start_2, Hdel_line_end_2,
        Hdel_speed_calculator, latest_speed_Hdel, unique_vehicle_count_Hdel, Hdel_roi
    ), media_type="multipart/x-mixed-replace; boundary=frame")
