...
import time
import asyncio
from speed_calculatorcopy import SpeedCalculator
import os
...
async def process_video(video_path, speed_calculator, latest_speed_store):
...
async def stream_video(video_name: str):
    """Starts processing a video file or IP camera and returns a video stream."""
    ...
    # 🔥 Detect if it's an IP camera
    if video_path.startswith(("rtsp://", "http://", "https://")):
        print(f"📡 Streaming live from IP Camera: {video_path}")
    else:
        print(f"🎥 Streaming recorded video: {video_path}")

    return StreamingResponse(process_video(video_path, PB_speed_calculator, latest_speed_PB), media_type="multipart/x-mixed-replace; boundary=frame")
...
def start_server() -> None:
    uvicorn.run("supabase_databse:app", host="0.0.0.0", port=8000)
